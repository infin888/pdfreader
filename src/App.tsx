...
export default function App(): JSX.Element {
  ...
  const [viewportSize, setViewportSize] = useState<{
    ...
    height: number;
    contentWidth: number;
  }>({ width: 0, height: 0, contentWidth: 0 });
  ...
  const firstTokenByPageRef = useRef<Map<number, HTMLElement>>(new Map());

  const hasDocument = tokens.length > 0;

  const pageWidth = useMemo(() => {
    const usableWidth = viewportSize.contentWidth > 0 ? viewportSize.contentWidth : viewportSize.width;
    return Math.max(1, usableWidth + COLUMN_GAP_PX);
  }, [viewportSize.contentWidth, viewportSize.width]);
  ...
  const scrollToPage = useCallback(
    (index: number, behavior: ScrollBehavior = 'smooth') => {
      ...
      const targetLeft = clampedIndex * pageWidth;
  ...
  const captureAnchor = useCallback(() => {
    ...
    return findAnchorTokenIndex(orderedTokenElementsRef.current, viewportRef.current, pageIndex, pageWidth);
  }, [pageIndex, pageWidth]);
  ...
  const updateWordsForPage = useCallback(
    (page: number) => {
      ...
      const count = countWordsForPage(orderedTokenElementsRef.current, viewportRef.current, page, pageWidth);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      frame = window.requestAnimationFrame(() => {
        ...
        const currentPage = Math.floor((viewport.scrollLeft + width / 2) / width);
        setPageIndex((prev) => {
          if (prev === currentPage) {
            return prev;
          }
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      const contentWidth = Math.max(0, viewport.clientWidth - paddingLeft - paddingRight);
      setViewportSize({ width: viewport.clientWidth, height: viewport.clientHeight, contentWidth });
    };
    updateSize();
    let frame = 0;
  ...
  useLayoutEffect(() => {
    ...
    if (!flow || !viewport) {
      ...
      firstTokenByPageRef.current = new Map();
      setPageCount(1);
      setWordsOnPage(0);
      return;
    }

    const { ordered, byIndex } = collectTokenElements(flow);
    ...
    const pageSpan = Math.max(1, pageWidth);

    const firstByPage = new Map<number, HTMLElement>();
    for (const el of ordered) {
      const page = getElementPage(el, viewport, pageSpan);
      el.dataset.pageIndex = String(page);
      if (!firstByPage.has(page)) {
        firstByPage.set(page, el);
      }
    }

    for (const el of ordered) {
      const page = el.dataset.pageIndex ? Number(el.dataset.pageIndex) : null;
      if (page != null && firstByPage.get(page) === el) {
        el.dataset.pageFirst = 'true';
        el.style.setProperty('scroll-snap-align', 'start');
        el.style.setProperty('scroll-snap-stop', 'always');
      } else {
        el.removeAttribute('data-page-first');
        el.style.removeProperty('scroll-snap-align');
        el.style.removeProperty('scroll-snap-stop');
      }
    }

    firstTokenByPageRef.current = firstByPage;

    const newPageCount = getPageCount(viewport, flow, pageSpan, ordered);
    ...
    if (pendingPageRef.current != null) {
      ...
      const anchorPage = getTokenPageIndex(anchorTokenRef.current, tokenElementMapRef.current, viewport, pageSpan);
      if (anchorPage != null) {
  ...
  }, [fontSize, pageIndex, scrollToPage, tokens, updateWordsForPage, viewportSize.height, viewportSize.width, pageWidth]);
  ...
  const columnWidthValue = useMemo(() => Math.max(1, pageWidth - COLUMN_GAP_PX), [pageWidth]);
          ...
          >
            <div
              ...
              style={{
                ...
                columnWidth: `${columnWidthValue}px`,
